...
- 클래스의 내부 구현을 리팩터링 등으로 변경했을 때 클라이언트가 영향을 받는가?
- 코드를 자주 변경해야 하는가? 그렇다면 인터페이스와 같은 더 안정적인 추상화 뒤에 코드를 숨기는 방식으로 디자인 할 수 있는가?
...
> - 새 메서드가 많은 파라미터나 클래스의 의존서응ㄹ 의존하는가? 개발자가 새 메서드의 요구사항을 빠르게 이해할 수 있을 만큼 간결한가?
> - 메서드가 호출될 때 구현을 살펴보지 않아도 이름만으로 기능을 충떄히 설명할 수 있는가?
...
> 코드를 언제 함께 유지해야 할까?
> - 둘 이상의 코드 조각이 독립적으로 존재할 수 없을 떄, 강제로 분리하면 메서드 시그니처가 복잡해질 수 있다.
> - 추출 대상이 교체 가능성이 낮을 때
> - 추출 대상의 코드를 완전히 따로 떼어 테스트할 만한 가치가 없을 때
> - 클래스 내, 비공개 메서드가 얼마 없을 떄
...
> - 클래스 이름이 개념과잘 들어 맞고 개념을 대표할 만한가?
