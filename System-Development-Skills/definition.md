...
- 코드는 매우 선언적이고 명시적이어야 한다.
- 선언적 코드는 "무엇을" 원하는지에 집중하고, "어떻게" 동작하는지는 감춘다.
- 코드의 의도가 명확하게 드러나야 하며, 읽는 사람이 쉽게 이해할 수 있어야 한다.
- 언어나 프레임워크의 숨겨진 동작(마법 같은 동작, 암묵적 처리)을 남용하면 예측 불가능한 결과를 초래할 수 있으므로 주의해야 한다.
- 의도치 않은 해결책이나 지나치게 추상화된 마법 같은 코드는 유지보수성과 신뢰성을 해칠 수 있다.
...
1. **가독성**: 코드의 의도가 명확하게 드러나 읽기 쉽다.
2. **유지보수성**: 세부 구현이 감춰져 있어 변경이 쉬워진다.
3. **재사용성**: 추상화된 연산을 조합해 다양한 문제를 쉽게 해결할 수 있다.
4. **예측 가능성**: 코드가 명확하게 동작하므로, 예상치 못한 부작용이 줄어든다.

### 주의사항

- 선언적 코드가 지나치게 추상적이거나, 언어나 프레임워크의 숨겨진 동작(예: 리플렉션, 암묵적 변환 등)에 의존하면 오히려 코드의 의도가 불분명해질 수 있다.
- 항상 코드의 의도와 동작이 명확하게 드러나도록 작성해야 하며, "마법처럼 보이는" 코드는 최소화해야 한다.