...
테스트의 이름은 문제가 무엇인지 설명해줄 뿐망아니라, 테스트의 의도를 표현할 수 있는 가장 간단한 방법이다.
...
복잡성은 대체로 논리(logic) 의 형태로 나타난다. 프로그래밍 언어에서의 논리는 명령형 요소(연산자, 반복문, 조건문등)을 이용해 표현한다. 논리가 포함된 코드의 결과를 예측하기 위해서는 약간의 정신 노동이 필요하다. 따라서 테스트에서 논리가 조금만 들어가도 추론의 난이도가 올라가게된다.
...
> ### **실패 매세지를 명확하게 작성하자.**
*잘 작성된 실패 메시지라면 테스트의 이름과 거의 동일한 정보를 담고 있어야 한다. 즉 '원하는 결과', '실제 결과', '이때 건네진 매개변수의 값' 을 명확하게 알려줘야한다.*
...
대부분의 소프트웨어는 '반복하지 말라(Don't repeat Yourself' 라는 뜻의 DRY 원칙을 숭배한다. DRY 는 개념들을 각각 독립된 하나의 장소에서 구현하여 코드 중복을 최소로 줄이면 유지보수하기 쉽다 말한다. 하지만  참조에 참조를 따라가야 실제 로직을 구현한 코드를 찾아 분석할 수 있기 때문에 '코드 명확성' 이 떨어진다는 단점이 생간다.
...
3. 뿐만아니라 도우미 메서드도 로직들로 가득 차 있어 무슨일을 하는지 한눈에 들어오지 않는다.
...
2. 이 코드를 읽는 사람은 각 테스트가 의도한 작업을 제대로 수행하고 있으며 숨겨진 버그도 없으리라 확신할 것이다.
...
잘 만들어진 검증용 도우미 메서드는 여러 조건을 확인하는 것이 아닌 입력에 대한 단 하나의 '개념적 사실'만을 검증한다.(개념을 검사하는 로직이 복잡할 경우라면 특히 큰 도움이 된다.)
