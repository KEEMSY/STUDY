...
가짜 객체의 가장 좋은 예시는 인메모리 데이터 베이스르 말할 수 있다.
...
스텁의 간단한 예시를 이야기 한다면 대상 함수가 반환할 값을 지정한다고 할 떄, 이를 '반환값을 뭉갠다.(스텁한다.)' 라고 말할 수 있다.
...
스텁은 일반적으로 모의 객체 프레임워크를 이용해 수행한다. 만약 모의 객체 프레임워크가 없다면 원하는 값을 반환하도록 하드코딩한 클래스들을 직접 생성하느라 상당량의 보일러 플레이트 코드를 작성해야 할 것이다.
...
상호작용 테스트는 함수가 올바른 방식으로 호출되지 않으면 실패하는 테스트를 검증하기 위해 존재한다고 말할 수 있다. 
...
```
상호작용테스트에서도 스텁와 유사하게 주로 모의객체 프레임워크를 사용한다.(모의객체 프레임워크가 없다면 함수가 몇번 호출되고 어떤 인수를 받았는지를 추적하는 새로운 클래스를 직접 작성해야한다.)
...
실제 구현을 선호하는 테스트 방식을  고전적 테스트(classical test)라고 하며 반대로 모의 객체 프레임워크를 선호하는 테스트방식은 모의 객체 중심주의 테스트(mockist test) 라고 한다.

모의 객체 중심주의에서는 엔지니어들이 대상 시스템을 엄격한 지침에 따라 설계해야한다.
...
하지만 실제 구현을 이용하려면 테스트를 위해 실제 구현은 물론 그 구현이 참조하는 코드들까지 빌드해야한다는 단점이 존재한다.
...
*같은 버전의 시스템을 대상으로 실행하면 언제든 똑같은 결과를 내어주는 테스트를 결정적(deterministic) 인 테스트라고한다.*
...
밀폐되지 않은, 즉 테스트가 통제할 수 없는 외부 서비스에 의존하는 코드는 비결정성의 주범으로 이야기 할 수 있다.
...
가짜 객체는 실제 구현과 비슷하게 동작하도록 만들어야 하기 때문에 노력도 더 들고 도메인 지식도 더 많이 필요하다. 뿐만아니라 실제 객체의 행위가 변경될 때마다 발맞춰 갱신해야 하므로 유지보수도 신경써야한다.

따라서, 가짜 객체를 사용할 사람이 많지 않다면 굳이 시간을 들일 이유는 없다.

<br>

유지 보수할 가짜 객체 수를 줄이려면 우선 테스트에서 진짜 객체를 사용하지 못하게 만드는 근본 원인을 찾는다. 그런 다음 해당 코드만 가짜 객체로 만든다.
...
## **뭉개기(스텁)**
*스텁은 원래는 없는 행위를 테스트가 함수에 덧씌우는 방법을 말한다.*
...
스텁을 과용할 경우 테스트를 유지보수할 일이 늘어나기 때문에 생산성을 갉아 먹을 수 있으므로 너무 많이 사용하지 않도록 주의하도록 하자.
...
1. **불명확해진다.**
    ...
    스텁은 대상 함수에 행위를 덧씌우는 코드를 추가로 작성하여 사용한다. 이는 테스트의 의도를 파악하기 어렵게 한다. 특히 대상 시스템이 어떻게 동작하는지 잘 모르는 사람에게는 이해하기 어려운 코드가 만들어질 것이다.

2. **깨지기 쉬워진다.**

    스텁을 이용하면 대상 시스템의 내부 구현방식이 테스트에 들어나게 된다. 즉 제품의 내부가 다르게 구현되면 테스트 코드도 함께 수정되야함을 뜻한다. 좋은 테스트라면 사용자에게 영향을 주는 공개 API가 아닌 한, 내부가 어떻게 달라지든 영향받지 않아야한다.

3. **테스트 효과가 감소한다.**

    스텁으로 원래 행위를 뭉개버리면 해당 함수가 실제 구현과 똑같이 동작한다 보장할 방법이 없다. 실제 구현의 명세에 의존하는 시스템이라면 스텁을 사용하지 않는 것이 좋다. 

    <br>

    또한 스텁을 이용하면 상태를 저장할 방법이 사라져, 대상 시스템의 특성 일부를 테스트 하기 어려울 수 있다. 
    - 실제 구현이나 가짜객체의 경우 `database.save(item`으로 저장한 상품정로를 `database.get(item.id())`를 통해 호출하여 다시 꺼내올 수 있지만 스텁에서는 불가능하다.
...
```
스텁을 사용하지 않을 경우 트랜잭션이 처리되는 자세한 과정이 테스트 코드에서 사라졌다.(처리 방법은 신용카드 서버가 알고 있으니 특별히 설정할 필요가 없다.)

<br>

신용카드 서버는 가짜 객체로 대체하는게 좋다. 가짜 객체를 이용할 수 없다면, 대안으로 실제 구현이 테스트용으로 밀폐시킨 신용카드 서버와 통신할 수 있도록 하자.(물론 가짜 객체보다는 속도가 느려질 것이다.)

<br>

> **스텁이 적합한 경우**
*스텁은 실제 구현을 포괄적으로 대체하기 보다는 특정함수가 특정 값을 반환하도록 하여 대상 시스템을 원하는 상태로 바꿀 때 사용하자.*

실제 구현이나 가짜 객체로는 원하는 결과값을 얻거나 특정 오류를 일으키기가 불가능하다. 하지만 스텁으로는 함수의 동작을 테스트 코드에서 정의 할 수 있으므로, 쉽게 원하는 결과값을 얻을 수 있다.

<br>

목적이 분명하게 들어나기 위해서는 스텁된 함수 하나하나가 단정문들과 직접적인 연관이 있어야 한다.

즉 적은 수의 함수만 스텁으로 대체한다.(스텁된 함수가 많을 수록 테스트의 의도는 희미해질것이다.) 

<br>

*스텁을 활용하기 괜찮은 상황일지라도 되도록 실제 구현이나 가짜 객체를 이용하도록 하자.*
...
상호작용 테스트는 모의 객체 프레임워크를 활용하면 어렵지 않게 수행할 수있지만. 테스트를 가치있고, 잘 읽히고, 변경하기 쉽게끔 관리하기 위해선는 정말 꼭 필요할 때만 적용하는 것이 좋다.
...
상호작용 테스트의 문제점은 크게 두가지를 이야기 할 수 있다.
...
상호작용 테스트가 필요한 경우는 다음과 같다.
- 실제 구현이나 가짜 객체를 이용할 수 없어서(실제 구현은 너무느리고 가짜 객체는 존재하지 않는상황) 상태 테스트가 불가능한 경우
- 함수 호출 횟수나 호출 순서가 달라지면 기대와 다르게 동작하는 경우(상태 테스트로는 검증하기 어려운 상황)

<br>

상호작용 테스트는  상태 테스트틑 완전히 대체하지 못한다. 따라서 단위 테스트에서 상태 세트스를 수행할 수 없다면 상호작용 테스트를 추가하는 대신 더 큰 범위의 테스트 스위트에서 상태 테스트를 수행하여 보완하는 것이 좋다.
...
일반적으로 상호작용 테스트는 상태 변경 함수에 한에서만 수행해야 한다. 그렇지 않는다면 다른 테스트와 중복될 가능성이 높다.(상호작용자체에는 부수효과가 없으므로 정확성 측면이 중요한 요소가 아니다.)
...
단위테스트는 메서드보다 행위를 테스트 하는 것이 더 좋다. 즉 테스트 하나가 여러 행위를 검증하려 시도하는 것보다 대상 메서드나 클래스가 제공하는 행위 하나만을 검증 하는데 집중해야한다.

<br>

어떤 함수들이 어떤 인수들을 받아 호출되는지를 너무 세세하게 검증하지 않느 ㄴ것이 좋다. 그래야 테스트가 더 명확해지고 간결해질 수 있다. 더 나아가 각 테스트의 범위를 벗어난 행위로 인한 변경의 영향을 덜 받으므로 함수 호출 방식이 달라져 실패하는 테스트의 수도 줄일 수 있다.
...
테스트 대역을 활용하면 대상 코드를 포괄저긍로 검증하고 테스트속도를 높여줘서 엔지니어링 속도에 아주 중요하다. 하지만 잘못 사용하면 테스트를 불분명하고, 깨지기 쉽고, 덜 효과적으로 만들어서 오히려 생산성을 크게 떨어트릴 수도 있다.

<br>

테스트 대역은 테스트에서 사용하기 어려운 의존성 문제를 멋지게 우회하게 도와준다. 하지만 제품 코드에 대한 확신을 극대화하려면 어느 지금에서는 실제 의존성을 적용하여 테스트를 해야한다. 그리고 이런 테스트는 더 큰 범위의 테스트에서 테스트한다.
