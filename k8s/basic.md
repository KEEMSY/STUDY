...
- 오브젝트(Object)
    ...
    2. 네임스페이스(Namespace): 쿠버네티스 클러스터에서 사용되는 리소스들을 구분해 관리하는 그룹을 말한다. 특별히 지정하지 않으면 기본으로 할당되는 defaul, 쿠버네티스 시스템에서 사용되는 kube-system, 온프레미스에서 쿠버네티스를 사용할 경우 외부에서 쿠버네티스 클러스터 내부로 접속하게 도와주는 컨테ㅣㅇ너들이 속해 있는 metallb-system이 존재한다.
